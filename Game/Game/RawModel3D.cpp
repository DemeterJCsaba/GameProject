...
void RawModel3D::loadFromOBJ(string fileName) {	
	...
	while (getline(in, line)) {
		...
		if (line[0] == 'v' && line[1] == ' ') {
			iss >> temp >> s1 >> s2 >> s3;
			m_Vertices.push_back(vec3(atof(s1.c_str()), atof(s2.c_str()), atof(s3.c_str())));
		}
		// Texture Coordinates
		...
		// Index Coordinates
			...
			m_IndicesVertex.push_back(atoi(s31.c_str())-1);
			m_IndicesNormal.push_back(atoi(s33.c_str())-1);
			m_IndicesColor.push_back(0);
			m_IndicesVertex.push_back(atoi(s21.c_str())-1);
			m_IndicesNormal.push_back(atoi(s23.c_str())-1);
			m_IndicesColor.push_back(0);
			m_IndicesVertex.push_back(atoi(s11.c_str())-1);
